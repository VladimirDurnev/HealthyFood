...
export const fetchRandom = createAsyncThunk("data/fetchRandom", async () => {
    const { data } = await axios.get(
        `https://api.edamam.com/api/recipes/v2?type=public&app_id=${app_id}&app_key=
        ${app_key}&calories=0-150&health=alcohol-free&random=true&imageSize=LARGE&excluded=drinks&time=5%2B`
    );
